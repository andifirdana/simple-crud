...
(function () {
  ...
  // ====================================================
// GANTT / TIMELINE PROYEK
// ====================================================

function renderGantt(computed) {
  ...
  if (!validItems.length) {
    ...
    ganttBody.innerHTML = `
      <tr>
        <td class="gantt-name">
          Belum ada Target Start dan Target End.
        </td>
      </tr>
    `;
  ...
  weeks.forEach(
    week => {
      ...
      const key =
        `${week.start.getUTCFullYear()}-${week.start.getUTCMonth()}`;
  ...
  head.innerHTML = `
    ...
    <tr>
      ...
      ${
        weeks.map(
          (_, index) => `
            <th class="gantt-week gantt-week-header">
              ${index + 1}
            </th>
          `
        ).join("")
